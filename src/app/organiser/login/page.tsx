...
export default function OrganiserLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await login(email, password, 'ORGANISER');
      if (!res.success) {
    ...
    <div style={containerStyle}>
      <div className="card" style={cardStyle}>
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <h2 style={{ color: '#fff', fontSize: '24px', fontWeight: 800 }}>Organiser Dashboard</h2>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Sign in to manage events, pricing, and ticket sales.</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input
              ...
              placeholder="organiser@domain.com"
              required
            />
          </div>
          ...
          <button
            ...
            disabled={submitting}
          >
            {submitting ? 'Authenticating...' : 'Sign In as Organiser'}
          </button>
        </form>

        <p style={registerPromptStyle}>
          Want to host events?{' '}
          <Link href="/organiser/register" style={{ color: 'var(--primary)', fontWeight: 600 }}>
